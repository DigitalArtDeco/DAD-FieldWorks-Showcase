# Public documentation

This directory contains public companion notes for the DAD FieldWorks Showcase.

## Current development preview

- [Current capabilities](current_public_status.md)
- [Claim boundaries](claim_boundaries.md)
- [Native workflow screenshot provenance](native_workflow_screenshot_provenance.md)
- [September 2026 website refresh review](showcase_refresh_2026_09.md)
- [Evidence contract architecture](evidence_contract_architecture.md)
- [Legal site identity audit](legal_site_identity_audit.md)
- [Publication notes](publication_notes.md)

## Historical visual records

These records and their original assets retain their original dates, provenance, descriptions and hashes. Former publication-role wording describes an earlier site state, not the current homepage.

- [Canonical-Yee field visualization provenance](canonical_yee_field_visualization_provenance.md)
- [FDTD ringdown hero provenance](fdtd_microwave_resonator_ringdown_clean_hero_provenance.md)
- [FDTD microwave resonator explanation](fdtd_microwave_resonator_explanation.md)
- [Visualization provenance](visualization_provenance.md)

The public notes summarize bounded capabilities without releasing the private implementation.

Copyright (c) 2026 DigitalArtDeco Labs UG (haftungsbeschränkt). All rights reserved.
